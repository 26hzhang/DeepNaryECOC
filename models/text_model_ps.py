...
def highway_layer(inputs, num_unit, activation, use_bias=True, reuse=tf.AUTO_REUSE, name="highway"):
    with tf.variable_scope(name, reuse=reuse, dtype=tf.float32):
        trans_gate = tf.layers.dense(inputs, units=num_unit, use_bias=use_bias, activation=tf.sigmoid,
                                     name="trans_gate", trainable=False)
    ...
    with tf.variable_scope(name, reuse=reuse, dtype=tf.float32):
        ...
        for i, (kernel_size, filter_size) in enumerate(zip(kernel_sizes, filters)):
            weight = tf.get_variable("filter_%d" % i, shape=[1, kernel_size, dim, filter_size], dtype=tf.float32,
                                     trainable=False)
            bias = tf.get_variable("bias_%d" % i, shape=[filter_size], dtype=tf.float32, trainable=False)
...
def bidirectional_rnn(inputs, seq_len, training, num_units, drop_rate=0.0, activation=tf.tanh, concat=True,
                      reuse=tf.AUTO_REUSE, name="bi_rnn", trainable=True):
    with tf.variable_scope(name, reuse=reuse, dtype=tf.float32):
        ...
        if concat:
            ...
            bias = tf.get_variable(name="bias", shape=[num_units], dtype=tf.float32, trainable=trainable)
    ...
    with tf.variable_scope(name, reuse=reuse, dtype=tf.float32):
        ...
        for layer in range(num_layers):
            ...
            output = bidirectional_rnn(outputs, seq_len, training, num_units, drop_rate, activation, concat=concat,
                                       reuse=reuse, name="bi_rnn_{}".format(layer), trainable=trainable)
...
class TextModel:
    def __init__(self, config, num_class, word_dict, char_dict, vectors, ckpt_path, pretrained_model):
    ...
    def _restore_part_weights(self, model_path):
        ...
        for v in variables:
            if "word_embedding" in v.name:
                ...
                variables_to_restore.append(v)
            elif "bi_rnn_model/bi_rnn_0" in v.name:
                variables_to_restore.append(v)
            elif "bi_rnn_model/bi_rnn_1" in v.name:
                variables_to_restore.append(v)
            else:
                continue
    ...
    def _build_model(self, vectors):
        ...
        with tf.variable_scope("word_embedding"):
            ...
            unk = tf.get_variable(name="unk", shape=[1, self.cfg.word_dim], dtype=tf.float32, trainable=False)
        ...
        train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                       scope="bi_rnn_model/bi_rnn_2|pooling_layer|projection")
        optimizer = tf.train.AdamOptimizer(learning_rate=self.lr)
        if self.cfg.grad_clip is not None:
            grads, vs = zip(*optimizer.compute_gradients(self.loss, var_list=train_vars))
