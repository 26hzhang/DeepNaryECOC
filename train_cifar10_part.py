...
from models.cifar_cnn_ps import CifarCNN
...
pretrained_model = "ckpt/cifar10_cnn/cifar_cnn-6800"

if num_meta_class == 2:
    name = "cifar10_cnn_part_ecoc"
elif num_meta_class == num_class:
    name = "cifar10_cnn_part_ri"
elif 2 < num_meta_class < num_class:
    name = "cifar10_cnn_part_nary_ecoc_{}".format(num_meta_class)
else:
...
for i in range(num_classifier):
    ...
    model = CifarCNN(num_class=num_meta_class, ckpt_path=ckpt_path.format(i + 1), pretrained_model=pretrained_model)
    if config.training:
        model.train(x_train.copy(), y_train_, x_test.copy(), y_test_, batch_size=400, epochs=30)
