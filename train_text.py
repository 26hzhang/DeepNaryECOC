...
if config.task == "stsa":
    ...
    if num_classifier == 1:
        ...
        if num_meta_class == 2:
            name = "stsa_model_ecoc"
        elif num_meta_class == num_class:
            name = "stsa_model_ri"
        elif 2 < num_meta_class < num_class:
            name = "stsa_model_nary_ecoc_{}".format(num_meta_class)
        else:
    ...
    if num_classifier == 1:
        ...
        if num_meta_class == 2:
            name = "trec_model_ecoc"
        elif num_meta_class == num_class:
            name = "trec_model_ri"
        elif 2 < num_meta_class < num_class:
            name = "trec_model_nary_ecoc_{}".format(num_meta_class)
        else:
            raise ValueError("num_meta_class must in [2, num_class]!!!")
        save_path = "ckpt/{}/".format(name)
        ckpt_path = save_path + "{}/"
...
if num_classifier == 1:  # single model
    model = TextModel(config, num_class, word_dict, char_dict, vectors, ckpt_path=ckpt_path)
    if config.training:
        model.train(train_words, train_chars, train_labels, test_words, test_chars, test_labels)
    _, pred_labels = model.test(test_words, test_chars, test_labels, batch_size=200)
    np.savez_compressed(ckpt_path + "pred_labels.npz", embeddings=pred_labels)
    get_conf_matrix(pred_labels, None, test_labels, filename="assets/{}_sm_heatmap".format(name))
