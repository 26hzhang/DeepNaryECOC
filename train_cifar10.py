...
if num_classifier == 1:  # single model
    print("build model...")
    model = CifarCNN(num_class=10, ckpt_path="ckpt/cifar10_cnn/")
    if config.training:
        model.train(x_train, y_train, x_test, y_test, batch_size=200, epochs=30)
    pred_labels = model.test(x_test, y_test)
    np.savez_compressed("ckpt/cifar10_cnn/pred_labels.npz", embeddings=pred_labels)

else:  # ensemble model
    if num_meta_class == 2:
        name = "cifar10_cnn_ecoc"
    elif num_meta_class == num_class:
        name = "cifar10_cnn_ri"
    elif num_meta_class == 4:
        num_meta_class = 95
        name = "cifar10_cnn_nary_ecoc_{}".format(num_meta_class)
